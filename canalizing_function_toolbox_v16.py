...
def bool_to_poly(f,left_side_of_truth_table=[],prefix = 'x',indices=[]):
    ...
    for i in range(num_values):
        if f[i]==True:
            monomial = '*'.join([(prefix+'%i' % (j)) if entry==1 else ('(1-'+prefix+'%i)' % (j)) for j,entry in zip(indices,left_side_of_truth_table[i])])
...
def update_SDDS(F, I, N, X, P):
    Fx = np.zeros(N, dtype = int)
    for i in range(N):
        nextstep = F[i][bin2dec(X[I[i]])]
...
def update_many_times(F,I,initial_state,n_steps):
    N = len(F)
    for i in range(n_steps):
        initial_state = update(F,I,N,initial_state)
...
def num_of_steady_states_asynchronous(F,I,N,nsim=500,EXACT = False, left_side_of_truth_table = [], initial_sample_points = [],search_depth=50,SEED=-1,DEBUG=True): 
...
def steady_states_asynchronous_given_one_IC(F,I,nsim=500, initial_condition = 0,search_depth=50,SEED=-1,DEBUG=True):         
    if SEED == -1:
        SEED = int(random.random()*2**31)
    np.random.seed(SEED)    
        ...
        for jj in range(search_depth): #as long as we haven't reached an attractor state, keep updating
            ...
            if FOUND_NEW_STATE == False: #found a steady state in the asynchronous state space
                try: # check if this state is a known steady state
                    ...
                    break
        if FOUND_NEW_STATE == True:
            print(jj)
            break
        if DEBUG:
            print()
    if sum(basin_sizes)<(nsim):
        print('Warning: only %i of the %i tested initial conditions eventually reached a steady state. Try increasing the search depth. It may however also be the case that your asynchronous state space contains a limit cycle.' % (sum(basin_sizes),nsim))
    return (steady_states,len(steady_states),basin_sizes,transient_times,steady_state_dict,dictF,SEED)


def steady_states_asynchronous_given_one_IC_weighted(F,I,nsim=500, stochastic_weights = [],initial_condition = 0,search_depth=50,SEED=-1,DEBUG=False):         
    if SEED == -1:
        SEED = int(random.random()*2**31)
    np.random.seed(SEED)    
...
def get_exact_asynchronous_state_space(F,I,left_side_of_truth_table = []):
    N = len(F)
    if left_side_of_truth_table == []:
        left_side_of_truth_table = list(map(np.array,list(itertools.product([0, 1], repeat = N))))

    dictF = dict() #final dictionary will be of size N*2^N
    T = np.zeros((2**N,2**N))
    for j,iteration in enumerate(range(2**N)):
        for i in range(N):
            x = left_side_of_truth_table[iteration].copy()
            xbin = j
            fx_i = update_single_node(F[i],x[I[i]])
            if fx_i > x[i]:
                fxbin = xbin + 2**(N-1-i)
...
def num_of_attractors(F, I, N, nsim = 500, EXACT = False, left_side_of_truth_table = [], initial_sample_points = []):
    ...
    for i in range(nsim if not EXACT else 2**N):
        if check_if_empty(initial_sample_points):
        ...
        while True: #as long as we haven't reached an attractor state, keep updating
            ...
            queue.append(fxbin)
            xbin = fxbin
    return (attractors, len(attractors), basin_sizes, attr_dict, initial_sample_points if not check_if_empty(initial_sample_points) else sampled_points)
