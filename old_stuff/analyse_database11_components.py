...
for i in range(len(Fs)):
    F=Fs[i]
    n_constants = len(constantss[i])
    n_total = len(F)
    n_genes = n_total-n_constants
    edges=[]
    ...
    len_scc = np.array(list(map(len,scc)))
    dummy_len_scc = len_scc[len_scc<max(len_scc)]
    dummy_len_scc = dummy_len_scc[dummy_len_scc>1]
    
    res.append([models_loaded[i].split('_')[-1].split('.')[0],models_loaded[i],n_genes,len(scc),len(dummy_len_scc),max(len_scc),sum(len_scc==1),', '.join(list(map(str,dummy_len_scc))) if len(dummy_len_scc)>0 else ''])
    print(i,n_genes,len(scc),len(dummy_len_scc))
    if len(dummy_len_scc)>0:
        draw_scc_condensation(i,layout=nx.shell_layout,SAVE=True,savefile='connected_components_diagram_'+models_loaded[i]+'.pdf',title=models_loaded[i])
A = pd.DataFrame(res,columns=['PMID','long_identifier','number of genes','number components',"number 'non-trivial' components",'size largest component','number components of size 1','sizes of non-trivial components'])
A.to_excel('connected_components_diagram_all.xlsx')


        
def draw_scc_condensation(i,layout=nx.fruchterman_reingold_layout,SAVE=False,savefile='',title=''):
    F=Fs[i]
    n_constants = len(constantss[i])
    n_total = len(F)
    n_genes = n_total-n_constants
    ...
    node_sizes = np.array(len_scc)/n_total*5000+200
    labels = {}    
    for ii,node in enumerate(cG.nodes()):
        labels[node] = str(len_scc[ii])
    pos = layout(cG)
    f,ax=plt.subplots()
    nx.draw(cG,node_size=node_sizes,labels=labels,node_color='orange',pos=pos,ax=ax)
    ax.set_title(title)
    if SAVE:
